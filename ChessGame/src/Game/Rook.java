...
/**
 * 
 * A bástya bábut képviselő osztály.
 *
 */
public class Rook extends Piece {
	private static final long serialVersionUID =8103269026356511386L;
	
	/**
	 * A bástya osztály konstruktora.
	 * 
	 * @param xp
	 * @param yp
	 * @param colour
	 ...
	 * @param p2c
	 */
	public Rook(int xp, int yp, boolean colour, String n, List<Piece> ps, boolean b, String p1, String p2, boolean p1c, boolean p2c) {
		super(xp, yp, colour, n, ps, b,p1,p2,p1c,p2c);
	}
	
	/**
	 * A bástya bábuk mozgatását végzi, az új helyet is módosítja, valamint a listát is megfelelően frissíti.
	...
	public void move(int xp,int yp){//kommentek a futó osztályban
        ...
        if(p!=null){
            if(p.getColour()!=this.getColour()){
            	if(p.getXP()==this.getXP()||p.getYP()==this.getYP())
            		p.kill();
            	else {
            		x=this.getXP()*64;
                    y=this.getYP()*64;
	...
	/**
	 * Visszaadja, hogy az adott bástya üti-e az ellentétes színű királyt.
	...
	public boolean canHitOppositeKing(int oppkingxp, int oppkingyp) {
		if(!(this.getXP()==oppkingxp||this.getYP()==oppkingyp))
			return false;
	...
	/**
	 * Visszaadja, hogy az adott bástya az adott pozícióba léphet-e
	 * A "move" függvényhez hasonlóan vizsgálja a különböző lehetséges eseteket.
